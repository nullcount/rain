...
from const import MESSAGE_SIZE_MB, LOG_ERROR, LOG_INFO, LOG_TRUSTED_SWAP_SERVICE as logs
import config
...
class Lnd(BitcoinLightingNode):
    ...
    def open_channel(self,channel: OpenChannelRequest) -> Result[None, str]:
        ...
        if isinstance(result,Ok):
            ...
            try:
                ...
                return Err(e.debug_error_string())
        config.log(LOG_INFO, msg.ok.format('lnd', channel))
    ...
    def get_pending_open_channels(self) -> Result[List[PendingOpenChannel], str]:
        ...
        config.log(LOG_INFO, msg.ok.format('lnd')) # TODO log channel list, just the IDs maybe
    ...
    def send_onchain(self) -> Result[None, str]:
        ...
        raise NotImplementedError
        config.logs(LOG_INFO, msg.ok.format('lnd'))
        return Ok(None)
    ...
    def get_confirmed_balance(self) -> Result[int, str]:
        ...
        config.logs(LOG_INFO, msg.ok.format('lnd', confirmed))
        return Ok(confirmed)
