...
class Report:
    ...
    def get_profit_loss(self):
        ...
        invoices_90day = invoices.filter(lambda r: self.date_gte(r['settle_date'], filter_90day))
        ...
        payments_1day = payments.filter(lambda r: self.date_gte(r['creation_date'], filter_1day))
        onchain_txs = self.lndg.get_onchain()
        onchain_txs_90day = onchain_txs.filter(lambda r: self.date_gte(r['time_stamp'], filter_90day))
        onchain_txs_30day = onchain_txs.filter(lambda r: self.date_gte(r['time_stamp'], filter_30day))
        onchain_txs_7day = onchain_txs.filter(lambda r: self.date_gte(r['time_stamp'], filter_7day))
        onchain_txs_1day = onchain_txs.filter(lambda r: self.date_gte(r['time_stamp'], filter_1day))
        closures = self.lndg.get_closures()
        closures_90day = closures.filter(lambda r: r['close_height'] >= (block_height - DAY_BLOCKS * 90))
        closures_30day = closures.filter(lambda r: r['close_height'] >= (block_height - DAY_BLOCKS * 30))
        closures_7day = closures.filter(lambda r: r['close_height'] >= (block_height - DAY_BLOCKS * 7))
        closures_1day = closures.filter(lambda r: r['close_height'] >= (block_height - DAY_BLOCKS))
        forwards = self.lndg.get_forwards()
        forwards_90day = forwards.filter(lambda r: self.date_gte(r['forward_date'], filter_90day))
        forwards_30day = forwards.filter(lambda r: self.date_gte(r['forward_date'], filter_30day))
        forwards_7day = forwards.filter(lambda r: self.date_gte(r['forward_date'], filter_7day))
        forwards_1day = forwards.filter(lambda r: self.date_gte(r['forward_date'], filter_1day))
